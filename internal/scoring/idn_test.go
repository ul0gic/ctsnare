...
func TestScoreConfusableKeywords(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantMatched: []confusableMatch{{keyword: "*paypal", points: brandKeywordPoints}},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			matched := scoreConfusableKeywords(tt.domain, brand, generic)
...
func TestConfusable_NoDoubleCount(t *testing.T) {
	...
	matched := scoreConfusableKeywords("paypal.xn--80ak6aa92e.com",
		[]string{"paypal"}, nil)
