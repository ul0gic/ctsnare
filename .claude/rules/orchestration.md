...
- **ALL agents** update `.project/build-plan.md` (mark task statuses) and `.project/changelog.md` (log milestones) — this provides real-time visibility across agents during parallel phases
- Only the lead session modifies other `.project/` files (prd.md, tech-stack.md, issues/)
...
- Each agent runs verification scoped to their directory only — see the build plan's Build Verification Protocol table for exact commands per agent
- Full project verification runs ONLY at merge gates
