...
> | CLI | `cli-engineer` | Green | `internal/tui/`, `internal/cmd/`, `cmd/cert-hunter/` |
...
| `cmd/cert-hunter/main.go` | BE, CLI | Scaffold in Phase 1. Only CLI agent modifies after that (owns Cobra root wiring) |
...
| During Phase 2 -- CLI agent | `go build ./internal/tui/... ./internal/cmd/... ./cmd/cert-hunter/... && go vet ./internal/tui/... ./internal/cmd/... ./cmd/cert-hunter/... && go test ./internal/tui/... ./internal/cmd/...` | CLI in worktree |
| During Phase 4 -- QA agent | `go test -v -count=1 ./...` | QA in worktree |
| During Phase 4 -- OPS agent | `golangci-lint run ./... && go build -o cert-hunter ./cmd/cert-hunter` | OPS in worktree |
| At merge gates | `go build -o cert-hunter ./cmd/cert-hunter && go vet ./... && golangci-lint run ./... && go test ./...` | Lead session on merged main |
| Before release | `go build -o cert-hunter ./cmd/cert-hunter && go vet ./... && golangci-lint run ./... && go test -race -count=1 ./...` | Lead session |
...
| OK | Completed | Task finished |
| XX | Blocked | Cannot proceed due to external dependency |
| ?? | Has Blockers | Waiting on another task |
| RV | In Review | Pending review/approval |
| -- | Skipped | Intentionally not doing |
| .. | Deferred | Postponed to later phase/sprint |
| CL | Cloud Eligible | Can be offloaded to cloud VM |
| WT | Worktree | Requires worktree isolation |
...
|-------|-------|-----------|---------|----------|----------|--------|
| Phase 1: Foundation | 15 | 0 | 0 | 0 | 0% | BE |
| Phase 2: Core Engine | 28 | 0 | 0 | 0 | 0% | BE, CLI |
| Merge Gate 1 | 1 | 0 | 0 | 0 | 0% | Lead |
...
|--------|------|-------------|-------|
| !! | 1.1.1 | Initialize Go module: `go mod init github.com/ul0gic/cert-hunter` in project root. Create `go.mod` with Go 1.26. | BE |
| !! | 1.1.2 | Create directory structure: `cmd/cert-hunter/`, `internal/cmd/`, `internal/poller/`, `internal/scoring/`, `internal/profile/`, `internal/storage/`, `internal/tui/`, `internal/config/`. Each directory gets a placeholder `.go` file with the correct `package` declaration so the module compiles. | BE |
| !! | 1.1.3 | Create `cmd/cert-hunter/main.go` -- minimal entry point that calls `internal/cmd.Execute()`. Just enough to compile: imports `internal/cmd`, calls `Execute()`, exits on error. | BE |
| !! | 1.1.4 | Create `.gitignore` with entries: `cert-hunter` (binary), `*.db`, `*.sqlite`, `.env`, `dist/`, `coverage.out`, `*.prof`. | BE |
...
| !! | 1.1.6 | **BUILD CHECK** -- `go build ./cmd/cert-hunter && go vet ./... && golangci-lint run ./...` passes clean with zero warnings. | BE |
...
| !! | 1.2.3 | **BUILD CHECK** -- `go build ./cmd/cert-hunter && go mod verify` passes clean. Verify `go.sum` is populated. | BE |
...
| Status | Task | Description | Agent |
|--------|------|-------------|-------|
| !! | 1.4.1 | Create `internal/cmd/root.go`. Define `rootCmd` with Cobra: `Use: "cert-hunter"`, `Short: "Monitor Certificate Transparency logs for suspicious domains"`, `Long` with full description. Add persistent flags: `--config` (string, config file path), `--db` (string, database path override), `--verbose` (bool, enable debug logging). Create `Execute() error` function that runs rootCmd. Wire `cmd/cert-hunter/main.go` to call this. | BE |
| !! | 1.4.2 | **BUILD CHECK** -- `go build -o cert-hunter ./cmd/cert-hunter && ./cert-hunter --help` shows help output with the correct description and flags. `golangci-lint run ./...` passes clean. | BE |
...
| !! | 2.10.6 | **BUILD CHECK** -- `go build ./cmd/cert-hunter && go vet ./internal/cmd/... && golangci-lint run ./internal/cmd/...` passes clean. `./cert-hunter --help` shows all subcommands (watch, query, db, profiles). | CLI |
...
5. Run full verification: `go build -o cert-hunter ./cmd/cert-hunter && go vet ./... && golangci-lint run ./... && go test ./...`
...
3. `cmd/cert-hunter/main.go` -- CLI agent's version takes precedence (they own the entry point)
4. `internal/cmd/root.go` -- CLI agent's version takes precedence

### Post-Merge Verification
- [ ] `go build -o cert-hunter ./cmd/cert-hunter` succeeds
- [ ] `go vet ./...` reports zero issues
- [ ] `golangci-lint run ./...` reports zero issues
- [ ] `go test ./...` all tests pass
- [ ] `./cert-hunter --help` shows all subcommands
- [ ] `./cert-hunter watch --help` shows watch flags
- [ ] `./cert-hunter query --help` shows query flags
...
| !! | 3.1.5 | **BUILD CHECK** -- `go build -o cert-hunter ./cmd/cert-hunter && go vet ./... && golangci-lint run ./... && go test ./...` passes clean. | CLI |
...
| !! | 3.2.3 | **BUILD CHECK** -- `go build -o cert-hunter ./cmd/cert-hunter && go vet ./... && golangci-lint run ./... && go test ./...` passes clean. Manual smoke test: `./cert-hunter watch --headless --verbose` starts without error, Ctrl-C shuts down cleanly. | CLI |
...
| !! | 3.3.2 | **BUILD CHECK** -- `go build -o cert-hunter ./cmd/cert-hunter && go vet ./... && golangci-lint run ./... && go test -count=1 ./...` all pass. Binary runs and responds to --help, watch --help, query --help, db --help, profiles --help. | CLI |
...
**OFF LIMITS during this phase:** `go.mod`, `go.sum`, all non-test `.go` files, `.golangci.yml`, `.project/build-plan.md`
...
| Status | Task | Description | Agent |
|--------|------|-------------|-------|
| !! | 4.3.1 | Create `Makefile` with targets: `build` (go build -o cert-hunter ./cmd/cert-hunter), `test` (go test -race -count=1 ./...), `lint` (golangci-lint run ./...), `fmt` (gofmt -w .), `vet` (go vet ./...), `clean` (rm -f cert-hunter *.db coverage.out), `coverage` (go test -coverprofile=coverage.out ./... && go tool cover -html=coverage.out), `check` (build + vet + lint + test -- full CI suite), `run` (go run ./cmd/cert-hunter), `help` (print all targets). All targets use `.PHONY`. | OPS |
...
| !! | 4.3.4 | Create `.goreleaser.yml`: project name cert-hunter, builds from ./cmd/cert-hunter, GOOS/GOARCH matrix, archive format tar.gz (linux/mac) and zip (windows), checksum file, changelog from git log. | OPS |
...
| !! | 5.2.2 | Create `README.md` at project root: project name + one-liner, installation instructions (go install + binary download), quick start (cert-hunter watch, cert-hunter query), all subcommands with examples, configuration section (TOML config file location, all configurable options with defaults), built-in profiles list, architecture overview (data flow diagram from PRD), development section (make dev, make test, make lint), license. | DOC |
...
*Next Milestone: Phase 1 complete, begin parallel Phase 2*
